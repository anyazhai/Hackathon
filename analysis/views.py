...
def suggestions_page_view(request):
    ...
    tracks = sp.search(q='genre:' + model_favourite_music_genres[0], type='track', market='GB', limit=1, offset=0)
    print(tracks)
    return render(request, 'analysis/suggestions.html', {'profile_model': profile_model.personality, 'tracks': tracks, 'failure': failure, 'hobby': hobby, 'final_result': final_result, 'response': response})
